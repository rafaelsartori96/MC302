...
public abstract class Carona {
	private final ArrayList<CaronaCaroneiro> caroneiros;
	...
	private float valor;
	private final ArrayList<MetodoPagamento> formaPagAceitas;
	...
	public boolean adicionarFormaPagamento(MetodoPagamento metodoPagamento) {
		if (formaPagAceitas.contains(metodoPagamento)) {
			return false;
		}
		
		formaPagAceitas.add(metodoPagamento);
			
		return true;
	}
	
	public boolean removerFormaPagamento(MetodoPagamento metodoPagamento) {
		return formaPagAceitas.remove(metodoPagamento);
	}
	
	public boolean checarExistenciaFormaPagamento(MetodoPagamento metodoPagamento) {
		return formaPagAceitas.contains(metodoPagamento);
	}
	...
	public String toString(int numeroEspacos) {
		...
		if(!caroneiros.isEmpty()) {
			...
			for(CaronaCaroneiro caroneiro: caroneiros) {			
				...
				stringBuilder.append(espacos2 + "| - Avaliacao dada para a carona: " + caroneiro.getAvaliacao() + "\n");
			}
		...
		if(!formaPagAceitas.isEmpty()) {
			...
			stringFormaPag = formaPagAceitas.stream()
